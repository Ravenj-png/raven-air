...
if SENTRY_DSN and SENTRY_DSN.startswith('https://'):
    ...
    logger.warning("⚠️ Sentry disabled")
...
# FIXED: Simple Pydantic model without Optional/Field
...
def verify_recaptcha(token):
    ...
    try:
        ...
        if not secret:
            return True  # Skip if no key (dev mode)
    ...
    try:
        ...
        if smtp_user and smtp_pass:
            with smtplib.SMTP('smtp.gmail.com', 587) as s:
                ...
                s.send_message(msg)
        logger.info(f"Email sent for {sub.email}")
    except Exception as e:
...
def health():
    return jsonify({'status': 'healthy', 'redis_connected': USE_REDIS})

@app.route('/api/v1/csrf-token', methods=['GET'])
def get_csrf_token():
    token = jwt.encode({'exp': datetime.utcnow() + timedelta(hours=1)}, app.config['SECRET_KEY'], algorithm='HS256')
    return make_response(jsonify({'csrf_token': token}))
...
def get_weather():
    ...
    if not api_key:
        return jsonify({'temp': 26, 'desc': 'Cloudy', 'humidity': 65}), 200
    try:
        ...
        return jsonify({'temp': 26, 'desc': 'Weather unavailable', 'humidity': 65}), 200
...
def chat_endpoint():
    ...
    try:
        reply = call_grok_api([{"role": "user", "content": data.message}])
...
def submit_contact():
    ...
    db.session.add(sub)
    db.session.commit()
    threading.Thread(target=send_email_async, args=(sub,)).start()
